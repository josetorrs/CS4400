...
class QueryGUI:
	...
	def __init__ (self, root):
		...
		self.beginCalendar = DateEntry(self.dateFrame, date_pattern = "mm-dd-yyyy", maxdate = datetime.today(),
									   mindate = datetime(2006, 3, 21), textvariable = self.beginDateVar).pack()

		self.endDateLabel = tk.Label(self.dateFrame, text = "Choose End Date").pack()
		self.endDateVar = tk.StringVar()
		self.endCalendar = DateEntry(self.dateFrame, date_pattern = "mm-dd-yyyy", maxdate = datetime.today(),
									 mindate = datetime(2006, 3, 21), textvariable = self.endDateVar).pack(pady = 5)
