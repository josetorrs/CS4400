...
class TwitterSentimentGUI:
    ...
    def __init__(self, root):
        ...
        DateEntry(root, textvariable=self.begin_date, date_pattern='mm-dd-yyyy', mindate=datetime(2006, 3, 21), maxdate=datetime.today(), year=2006, month=3, day=21).grid(row=1, column=1, sticky=E)
        
        Label(root, text='End Date').grid(row=2, column=0, sticky=W)
        DateEntry(root, textvariable=self.end_date, date_pattern='mm-dd-yyyy', mindate = datetime(2006, 3, 21), maxdate=datetime.today()).grid(row=2, column=1, sticky=E)
    ...
    def send_query(self):
        ...
        self.result = backend(topic, begin_date, end_date, min_likes, min_retweets)

        
def main():
    root = Tk()
    TwitterSentimentGUI(root)
    root.mainloop()
